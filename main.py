...
async def process_taboo_command(message: Message):
    ...
    async with aiosqlite.connect('bot_nesibintelk.db') as database:
        ...
        await bot.send_message(message.chat.id,
                               f'слово "{word}" добавлено в список исключений, его нельзя '
                               f'будет добавить в таблицу bad_words')
        await database.commit()
...
async def holiday(message: Message):
    ...
    """действия при вызове комканды holiday- покажет список праздников сегодня"""
    async with aiosqlite.connect('bot_nesibintelk.db') as database:
        answer = await database.execute(f'Select celebrate from holiday where date = ?', (for_select,))
        answer = await answer.fetchall()
        answer_database = '\n'.join([i[0] for i in answer])
        await bot.send_message(message.chat.id, text=f'<b>Какой сегодня праздник</b>\n{answer_database}',
                               parse_mode='html')
        await database.commit()
...
async def weather(message: Message):
    ...
    # reference_time=2024-01-29 06:15:42+00:00, status=clouds, detailed_status=overcast clouds>
    temp = weathers.temperature('celsius')
    # observation = mgr.weather_at_place('Krasnodar')
    # weathers = observation.weather
    # {'temp': 2.32, 'temp_max': 2.32, 'temp_min': 2.32, 'feels_like': 2.32, 'temp_kf': None}
    # temp = weathers.temperature("celsius")
    # print(temp)
    await bot.send_message(message.chat.id, f'<b>Прогноз погоды на сегодня</b>:\nмаксимальная температура '
                                            f'<b>{round(temp["temp_max"])}°C</b>\nминимальная температура '
                                            f'<b>{round(temp["temp_min"])}°C</b>\n'
                                            f'температура сейчас <b>{round(temp["temp"])}°C</b>\n'
                                            f'ощущается как <b>{round(temp["feels_like"])}°C</b>', parse_mode='html')
    ...
    async with aiosqlite.connect('bot_nesibintelk.db') as database:
        ...
        if answer_database is not None:
            await bot.send_message(chat_id=sibintek,
                                   text=f'Сегодня свой день рождение празднует {answer_database[0]}! '
...
async def check_out_boys():
    async with aiosqlite.connect('bot_nesibintelk.db') as database:
        ...
        await bot.send_message(chat_id=-1001214772818, text=f'Больше всего сообщений с '
                                                            f'нецензурными словами за последние семь дней '
        ...
        await bot.send_message(chat_id=sibintek, text=f'Общая статистика: \n{text}')
...
async def holiday_send():
    ...
    async with aiosqlite.connect('bot_nesibintelk.db') as database:
        ...
        answer = await answer.fetchall()
        answer_database = '\n'.join([i[0] for i in answer])
        await bot.send_message(chat_id=my, text=f'<b>Какой сегодня праздник</b>\n{answer_database}',
                               parse_mode='html')
        await bot.send_message(chat_id=sibintek, text=f'<b>Какой сегодня праздник</b>\n{answer_database}',
                               parse_mode='html')
